...
# 漸層色組合（三色之間）
GRADIENT_3 = [COLOR_DARK_BLUE, COLOR_BLUE, COLOR_PINK]
PALETTE = [COLOR_DARK_BLUE, COLOR_BLUE, COLOR_PINK, "#5f9df7", "#ff8a9b", "#a8e6cf"]
...
def base_layout(height=320, legend=True):
    """Return a clean dark layout dict with rounded corners."""
    d = dict(
...
def chart_deviation(height=310):
    ...
    dev  = df_s["達成率"] - 80
    colors = [COLOR_DARK_BLUE if v >= 0 else COLOR_PINK for v in dev]
    fig = go.Figure()
...
def chart_top5(height=260):
    ...
    fig = px.bar(df_t, x="提出金額", y="姓名", orientation="h",
                 color="達成率", color_continuous_scale=[COLOR_PINK, COLOR_DARK_BLUE],
                 text="提出金額")
    fig.update_traces(
        texttemplate="%{text:,.0f}$", textposition="outside",
        marker=dict(line_width=0, opacity=0.8, cornerradius=10)
    )
    layout = base_layout(height, legend=False)
...
def chart_product_pie(height=260, hole=0.45):
    """圓餅圖 — 三色、可旋轉、冰塊質感、放大後字更大圖更小"""
    df_p = df_sales.groupby("產品名稱")["銷售金額"].sum().reset_index()
    
    # 顏色循環（三色為主）
    colors_cycle = [COLOR_DARK_BLUE, COLOR_BLUE, COLOR_PINK] * 10
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        textfont=dict(size=14, color="white", family="Noto Sans TC"),
        pull=[0.05]*len(df_p),
...
def chart_quarterly(height=360):
    ...
    colors_q = [COLOR_DARK_BLUE, COLOR_BLUE, COLOR_PINK, COLOR_PINK]
    fig = go.Figure()
    for i, q in enumerate(quarters):
        fig.add_trace(go.Bar(
            name=q, x=df_target["姓名"], y=df_target[q],
            marker=dict(color=colors_q[i], line_width=0, opacity=0.85, cornerradius=10),
        ))
    layout = base_layout(height)
...
def chart_annual(height=370):
    ...
    fig = px.bar(df_s, x="年度目標", y="姓名", orientation="h",
                 color="年度目標", color_continuous_scale=[COLOR_DARK_BLUE, COLOR_BLUE],
                 text="年度目標")
    fig.update_traces(
        texttemplate="%{text:,.0f}$", textposition="outside",
...
def chart_sales_by_person(height=370):
    ...
    fig = px.bar(df_b, x="銷售金額", y="業務員", orientation="h",
                 color="銷售金額", color_continuous_scale=[COLOR_DARK_BLUE, COLOR_BLUE],
                 text="銷售金額")
    fig.update_traces(
        texttemplate="%{text:,.0f}$", textposition="outside",
        marker=dict(line_width=0, opacity=0.85, cornerradius=10)
...
def chart_product_qty(height=370):
    ...
    fig = px.bar(df_q, x="產品名稱", y="數量",
                 color="數量", color_continuous_scale=[COLOR_DARK_BLUE, COLOR_BLUE],
                 text="數量")
    fig.update_traces(
...
def chart_gift_stacked(height=340):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(color=COLOR_PINK, line_width=0, opacity=0.85, cornerradius=8),
        text=[f"{v}%" for v in df_g["已領用%"]], textposition="inside",
        textfont=dict(color="white", size=11),
    ))
    fig.add_trace(go.Bar(
        name="剩餘", x=df_g["禮品名稱"], y=df_g["剩餘%"],
        marker=dict(color=COLOR_DARK_BLUE, line_width=0, opacity=0.85, cornerradius=8),
...
def chart_crm(height=300):
    ...
    fig = px.bar(df_c, x="維護內容", y="費用",
                 color="費用", color_continuous_scale=[COLOR_DARK_BLUE, COLOR_PINK],
                 text="費用")
    fig.update_traces(
        texttemplate="%{text:,.0f}$", textposition="outside",
        marker=dict(line_width=0, opacity=0.85, cornerradius=10)
